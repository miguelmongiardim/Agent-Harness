...
| `src/agent_harness/runtime.py` | Rewrite around salvageable pieces | It creates run artifacts and approvals, but dry-run is conflated with pending approval, resume does not complete run summary, and model/runtime boundaries are not yet proven. | Dry-run artifact test, tool-approval test, runtime-resume test. |
