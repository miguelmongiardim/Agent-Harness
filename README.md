...
evidence for review. Phase 3 provides `agent-harness evidence pack`, `check`,
...
`evidence_findings.v1`, and checksum files, plus redaction-safe artifact
indexing from V12 governance indexes. Absolute paths, traversal references, raw
...
domains are recorded as `not_present`. Markdown presentation, archive creation,
control mapping, operator routes, UI views, and release-readiness gates remain
later-phase work. The evidence pack does not certify compliance with any legal,
regulatory, security, or organizational framework.
