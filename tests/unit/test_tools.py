...
def test_patch_file_requires_bound_approval(tmp_path: Path) -> None:
    ...
    pending, _ = executor.execute(call, task, "checkpoint", run_id="run-1")
    ...
    applied, _ = executor.execute(call, task, "checkpoint", run_id="run-1", approval=approval)

    assert applied.success
    assert "def add(a: int, b: int) -> int" in target.read_text(encoding="utf-8")


def test_patch_file_rejects_approval_from_different_run(tmp_path: Path) -> None:
    target = tmp_path / "sample.py"
    original = "def add_numbers(a, b):\n    return a + b\n"
    target.write_text(original, encoding="utf-8")
    profile = PolicyProfile.model_validate(DEFAULT_POLICY)
    ...
    with pytest.raises(ValueError, match="approval binding does not match action"):
        executor.execute(call, task, "checkpoint", run_id="run-1", approval=approval)
